...
def getMask(x):
    z = torch.zeros(x.shape, dtype=torch.float32).cuda()
    return torch.ne(x, z)

def setZeroToNum(input, target):
    Mask = getMask(target)
    ValidElement = torch.sum(Mask)

    ones = torch.ones(target.shape, dtype=torch.float32).cuda()
    zeros = torch.mul(torch.ones(target.shape, dtype=torch.float32), 100).cuda()

    _target = torch.add(torch.mul(Mask.float(), target),
                        torch.mul((ones - Mask.float()), zeros))

    return input, _target, Mask, ValidElement.float()


def evaluateError_M(output, target):
    errors = {'MSE': 0, 'RMSE': 0, 'ABS_REL': 0, 'LG10': 0,
              'MAE': 0,  'DELTA1': 0, 'DELTA2': 0, 'DELTA3': 0}

    _output, _target, Mask, ValidElement = setZeroToNum(output, target)

    if (ValidElement.data.cpu().numpy() > 0):
        diffMatrix = torch.abs(_output - _target)
        diffMatrix = torch.mul(diffMatrix, Mask.float())

        errors['MSE'] = torch.sum(torch.pow(diffMatrix, 2)) / ValidElement

        errors['MAE'] = torch.sum(diffMatrix) / ValidElement

        realMatrix = torch.div(diffMatrix, _target)
        realMatrix = torch.mul(realMatrix, Mask.float())
        errors['ABS_REL'] = torch.sum(realMatrix) / ValidElement

        LG10Matrix = torch.abs(lg10(_output) - lg10(_target))
        LG10Matrix = torch.mul(LG10Matrix, Mask.float())
        errors['LG10'] = torch.sum(LG10Matrix) / ValidElement
        ...
        errors['DELTA1'] = torch.sum(
            torch.le(maxRatio, 1.25).float()) / ValidElement
        errors['DELTA2'] = torch.sum(
            torch.le(maxRatio, math.pow(1.25, 2)).float()) / ValidElement
        errors['DELTA3'] = torch.sum(
            torch.le(maxRatio, math.pow(1.25, 3)).float()) / ValidElement
